...
int ShipState::isPossibleToBalance() const {
...
//    double min_X = balanceSum - abs(balanceSum - left_sum) / 2;
    ...
    int max_X = floor(sum / 1.9);       // round down, if above max_X no longer fits balance factor
    int min_X = ceil(sum * 0.9 / 1.9);  // round up, if below min_X no longer fits balance factor
